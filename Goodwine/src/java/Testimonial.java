...
public class Testimonial extends HttpServlet {
 ...
 throws ServletException, IOException { 
         ...
         if(!sermon.isEmpty()){
             if(!author.isEmpty()){
                  if(!topic.isEmpty()){
                      //it is video file proceed with uploading
                         ...
                         if(source.exists()){
                             ...
                             if(updateDatabase(target,target.getName(),sermon,author,topic,newsletter)){
                                  response.getWriter().write("testimonial uploaded successfully");
                      ...
                      response.getWriter().write("please enter tstimonial heading");
                  }
                 
             }else{
                 response.getWriter().write("please enter testifier name");
             }
         }else{
              response.getWriter().write("please enter full story of the testimony");
         }
  ...
  public boolean updateDatabase(File post,String fileName,String sermon,String author,String topic,
        ...
        String sql = "insert into post values(0,'testimonial','"+topic+"','"+author+"','"+fileName+"','"+sermon+"')";
        if(jdbc.update(sql)){
            done = true;
            // send newsletter
            String html = "<center>";
