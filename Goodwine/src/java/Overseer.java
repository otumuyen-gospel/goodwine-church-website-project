...
public class Overseer extends HttpServlet {
 @Override
 ...
 throws ServletException, IOException { 
     ...
     if(format.endsWith(".png") || format.endsWith(".jpg")|| format.endsWith(".jpeg") || format.endsWith(".gif")){
         if(!statement.isEmpty()){
             if(!name.isEmpty()){
                 ...
                 response.getWriter().write("please enter name of overseer");
             }
         }else{
  ...
  public boolean updateDatabase(String realPath,String fileName,String statement,String name){
        ...
        if(jdbc.getRowSize("about") > 0){
            try{
                 ResultSet rs  = jdbc.query("select overseer_image from about");
                 if(rs.next()){
            ...
            if(jdbc.update("update about set overseer_image='"+fileName+"', overseer_text='"+statement+"', "
                    + "overseer_name='"+name+"'")){
                //delete old file
            ...
            String sql = "insert into about values(0,'','','','','','','','','','','','"+name+"','"+fileName+"'"
            + ",'"+statement+"','','','','','','','','')";
            if(jdbc.update(sql)){
                done = true;
            }
