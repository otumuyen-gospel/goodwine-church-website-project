...
@WebServlet(urlPatterns = {"/Mission"})
...
public class Mission extends HttpServlet {
 ...
 throws ServletException, IOException { 
         ...
         if(!statement.isEmpty()){
             ...
             if(source.exists()){
                 ...
                 if(updateDatabase(resource.webUploadPath()
                         ,target.getName(),statement)){
                      response.getWriter().write("mission statement updated successfully");
              ...
              response.getWriter().write("please write a mission statement");
         }
  ...
  public boolean updateDatabase(String realPath,String fileName,String statement){
        boolean done = false;
        JDBC jdbc = new JDBC();
        jdbc.connect();
        ...
        if(jdbc.getRowSize("about") > 0){
            try{
                 ResultSet rs  = jdbc.query("select mission_image from about");
                 if(rs.next()){
            ...
            if(jdbc.update("update about set mission_image='"+fileName+"', mission_message='"+statement+"'")){
                //delete old file
                new File(realPath+oldFile).delete();
                done = true;
            }
        }else{
            String sql = "insert into about values(0,'','','','','"+statement+"','"+fileName+"','','','','','','',''"
            + ",'','','','','','','','','')";
            if(jdbc.update(sql)){
                done = true;
            }
