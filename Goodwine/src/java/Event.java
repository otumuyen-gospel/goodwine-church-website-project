...
public class Event extends HttpServlet {
 ...
 throws ServletException, IOException { 
         ...
         if(!sermon.isEmpty()){
             if(!author.isEmpty()){
                  if(!topic.isEmpty()){
                      //it is video file proceed with uploading
                         ...
                         if(source.exists()){
                             ...
                             if(updateDatabase(target,target.getName(),sermon,author,topic,newsletter)){
                                  response.getWriter().write("event uploaded successfully");
                      ...
                      response.getWriter().write("please enter topic/title of this event");
                  }
                 
             }else{
                 response.getWriter().write("please enter organizer/author for this event");
             }
         }else{
              response.getWriter().write("please enter detail description for this event");
         }
  ...
  public boolean updateDatabase(File post,String fileName,String sermon,String author,String topic,
        ...
        String sql = "insert into post values(0,'event','"+topic+"','"+author+"','"+fileName+"','"+sermon+"')";
        if(jdbc.update(sql)){
            done = true;
            // send newsletter
            String html = "<center>";
